...
class ProjectProductService {
  ...
  public getSpecifiedProductsByBrand = async (
    ...
    const zonesRoomIds = projectZones.reduce((totalIds: any, currZone: any) => {
      const zoneRoomIds = currZone?.areas?.reduce(
        (zoneIds: any, currArea: any) => {
          const roomsIds = currArea?.rooms?.reduce(
            (areaIds: any, currRoom: any) => {
    ...
    const filteredSpecifiedProducts = rawSpecifiedProducts.map(
      (brandProducts: any) => {
        const newProducts = brandProducts?.products?.filter(
          (product: any) =>
  ...
  public getSpecifiedProductsByMaterial = async (
    ...
    const specifiedProducts = rawSpecifiedProducts.filter(
      (product: any) =>
