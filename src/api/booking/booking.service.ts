...
import {ENVIROMENT, DefaultTimezone} from '@/config';
import {
...
export default class BookingService {
  ...
  public async create(payload: BookingPayloadRequest) {
    ...
    const schedule = await this.validationSchedule(payload.date, payload.slot);
    if (!schedule) {
  ...
  private async validationSchedule(date: string, slotTime: SlotTime) {
  ...
  private async sendEmail(datetime: string, booking: BookingAttributes) {
    ///
    const startTime = moment(`${datetime}+08:00`);
