...
interface EventReschedulePayloadRequest extends Pick<
...
class LarkOpenAPIService {
  ...
  private getAppAccessToken = async () => {
    try {
      const response = await axios.post(
        `${this.api_url}/open-apis/auth/v3/app_access_token/internal`,
        {
          app_id: this.app_id,
  ...
  public getEventList = async (
    ...
    return axios.get(
      `${this.api_url}/open-apis/calendar/v4/calendars/${this.calendar_id}/events`,
      {
  ...
  public updateEvent = async (
    event_id: string,
    data: EventReschedulePayloadRequest
    ) => {
    if (this.isExpiredToken()) {
      await this.getAppAccessToken();
    }
    ...
    return axios.patch(
      `${this.api_url}/open-apis/calendar/v4/calendars/${this.calendar_id}/events/${event_id}`,
      data,
      { headers: { Authorization: `Bearer ${this.tenant_access_token}` }}
    )
  }
