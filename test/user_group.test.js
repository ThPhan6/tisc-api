      ...
      chaiResponse(res, null, 200, ['data'], ['brands', 'pagination'], () => {
        res.body.data.brands.forEach((brand) => {
          brand.should.have.keys(
            "assign_team", "cards", "categories", "collections",
            "coverages", "created_at", "distributors", "id",
            "locations", "logo", "name", "origin", "products",
            "status", "status_key", "teams"
          );
        });
      });
    }).timeout(5000);
  ...
  describe("Design Firms", () => {
    it("Get List", async () => {
      const res = await chai
        ...
        .set({ Authorization: `Bearer ${token}` })
      chaiResponse(res, null, 200, ['data'], ['designers', 'pagination'], () => {
        res.body.data.designers.forEach((designer) => {
          designer.should.have.keys(
            "archived", "assign_team", "capacities", "created_at", "designers",
            "id", "live", "logo", "main_office", "name", "on_hold",
            "origin", "projects", "satellites", "status", "status_key"
          );
        });
      });
    }).timeout(5000);
