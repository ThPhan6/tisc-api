import Connection, {
  connection,
} from "@/Database/Connections/ArangoConnection";
...
class DatabaseConsole {
  ...
  private createTemplate = async (
    ...
    let fileTemplate = fs
      .readFileSync(`${this.templatePath}/${templateType}.template.ts`)
      .toString();
    const compiled = template(fileTemplate);
    fs.writeFileSync(
      `${this.databasePath}/${templateType}s/${moment().format(
        "YYYYMMDDHHmmss"
      )}_${filename}.ts`,
      compiled({ collection })
    );
  ...
  public runUp = async (collection: MigrationSeedModel) => {
    ...
    connection.collection(collection).save(
      unexecutedFiles.map((unexecutedFile) => {
        return {
          file: unexecutedFile,
          batch: latestBatch + 1,
        };
  ...
  public rollback = async () => {
    ...
    await this.executeTemplate(
      rollbackFiles.map((item) => item.file),
      "migrations",
      "down"
    );
    /// get record _keys
    ...
    connection
      ...
      .removeByKeys(keys, { waitForSync: true });
  };
  ...
  private executeTemplate = async (
    ...
    console.log("\x1b[35m");
    await Promise.all(
      unexecutedFiles.map(async (unexecutedFile) => {
        const fileModule = await import(
          `${this.databasePath}/${collection}/${unexecutedFile}`
        );
        if (isFunction(fileModule.up) && type === "up") {
          try {
            await fileModule.up(connection);
          } catch {}
        }
        if (isFunction(fileModule.down) && type === "down") {
          try {
            await fileModule.down(connection);
          } catch {}
        }
