...
const seed = async () => {
  const permissionDetailCollection = await db.collection("permission_details");
  const createAndSeed = async (collection) => {
    await collection.create();
    await collection.get();
    let records = [];
    const createDetailData = (permission) => {
  ...
  try {
    await createAndSeed(permissionDetailCollection);
  } catch (error) {
    if (error.message === "duplicate name") {
      await permissionDetailCollection.drop();
      await createAndSeed(permissionDetailCollection);
    }
  }
};
